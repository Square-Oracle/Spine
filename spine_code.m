Neck1 = imread('C:\Users\Daniel\Pictures\Neck-X-ray p1.jpg');
Neck2 = rgb2gray(Neck1);
%Neck3 = imcomplement(Neck2);
%imshow(Neck3);
Neck4 = edge(Neck2, 'Sobel',0.00888);
imshow(Neck4);
%thresh = multithresh(Neck2,2);
%seg_I = imquantize(Neck2,thresh);
%RGB = label2rgb(seg_I);
%%figure;
%imshow(RGB)
%axis off
%title('RGB Segmented Image')
function [e, thresh] = canny(Neck1, varargin)
...
inp.addParameter('SMethod', 'boundary');
inp.addParameter('SRadius', 1);
inp.addParameter('TMethod', 'none')
inp.addParameter('TValue', [], checkthresh)
inp.addParameter('TRatio', [], @(t) isempty(t) || ...
    checkattributes(t, {'numeric'}, ...
    {'nonnan' 'real' 'nonnegative' 'scalar' '<=' 1}));
inp.addParameter('TConn', []);
